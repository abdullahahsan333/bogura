...
<div class="w-full">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('admin.projects') }}" class="w-48 text-white bg-brand-btn font-semibold py-2 rounded-br-lg rounded-bl-lg rounded-tr-lg flex items-center gap-2 justify-center">
    ...
    <div class="bg-white overflow-auto mt-3">
        <form action="{{ route('admin.project.store') }}" method="POST" enctype="multipart/form-data" class="p-10 bg-white rounded shadow-xl">
            @csrf
            ...
            <div class="leading-loose">
                ...
                <div class="mt-2">
                    ...
                    <input class="w-full px-5 py-4 text-gray-700 bg-gray-200 rounded" autocorrect="off" spellcheck="false" autocomplete="off"
                        id="name" name="name" type="text" required="" placeholder="Project Name" aria-label="Name">
                </div>
                ...
                <div class="mt-2">
                    ...
                    <select id="empId" name="emp_id" class="employeeList w-full">
                        ...
                        @foreach ($employeeList as $employee)
                            <option value="{{ $employee->id }}">{{ strFilter($employee->name) }}</option>
                        @endforeach
                        @endif
                    </select>
                </div>
